...
void main() {
/*
Offset: offset to the left and offset to the right.
Weight: weight associated with its offset.

        o            o            o            o            o
    -offset[2]   -offset[1]    offset[0]    offset[1]    offset[2]
     weight[2]    weight[1]    weight[0]    weight[1]    weight[2]
...
*/
   ...
   if (uBlurLevel == 10) {
    ...
    for (int i = 1; i < 3; i++) {
        // Two pixels on the right
        result +=
        texture2D(sVideoTexture, clamp(vTexCoord + vec2(offset[i]/uWidth, 0.0), 0.0, 1.0)).rgb * weight[i];
